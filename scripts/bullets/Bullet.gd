...
onready var collisionLayers = preload("res://scripts/hitbox/CollisionLayers.gd").new().CollisionLayers
...
#	damage Characters
	if(body.collision_layer == collisionLayers.COLLISION_VALUE_HOSTILE or
	body.collision_layer == collisionLayers.COLLISION_VALUE_PLAYER):
		body.damage(damage)
	
#	remove on hit
	if(body.collision_layer == collisionLayers.COLLISION_VALUE_HOSTILE or
	body.collision_layer == collisionLayers.COLLISION_VALUE_PLAYER or
	body.collision_layer == collisionLayers.COLLISION_VALUE_WALL or
	body.collision_layer == collisionLayers.COLLISION_VALUE_DESTROYABLE):
		queue_free()
